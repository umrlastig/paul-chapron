...
Dans un espace rectilinéaire, i.e. où on utilise la **distance de Manhattan**: 
$$d(A,B)= \sum_{i}|A_i-B_i| \in \mathbb{R}$$
Avec A et B deux points, et $A_i$ la ième coordonnée du point A.
:::
...
```{r polar1, echo=FALSE, fig.height=4, fig.width=8, cache=TRUE}
set.seed(5678)  
ssd <-  read_sf("data/neufTrois_simple.shp") %>% st_as_sf()
par(mar=c(0,0,0,0))
par(mfrow=c(1,2))
...
myflows <- prepare_mat(nav, i = "i", j = "j", fij = "fij")
...
# each urban area.
flowSel1 <- select_flows(mat = myflows/rowSums(myflows)*100,  method="xfirst",
                       k = 20)


# Select the dominant flows (incoming flows criterion)
flowSel2 <- select_flows(mat = myflows, method="dominant",w = colSums(myflows), k = 1)
...
### Principe d'un modèle  de mobilité
